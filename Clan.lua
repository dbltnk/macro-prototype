	...
	onNew = function (self)
		...
		if self.hometown then
			...
			if not self.created_via_network then		
				self.testChar1 = Character:new{x= homeX + math.random(-50,50),y = homeY + math.random(-50,50), skillLevel = 2, XPLevel = 0, equipLevel = 0, loginTime = 16, logoutTime = 22}
				table.insert(self.members, self.testChar1)
				self.testChar1.clan = self.name
				self.testChar1.nr = 1
			
				self.testChar2 = Character:new{x= homeX + math.random(-50,50),y = homeY + math.random(-50,50), skillLevel = 1, XPLevel = 0, equipLevel = 0, loginTime = 12, logoutTime = 23}
				table.insert(self.members, self.testChar2)
				self.testChar2.clan = self.name
				self.testChar2.nr = 2
				
				self.testChar3 = Character:new{x= homeX + math.random(-50,50),y = homeY + math.random(-50,50), skillLevel = 1, XPLevel = 0, equipLevel = 0, loginTime = 20, logoutTime = 15}	
				table.insert(self.members, self.testChar3)
				self.testChar3.clan = self.name
				self.testChar3.nr = 3
				
				self.testChar4 = Character:new{x= homeX + math.random(-50,50),y = homeY + math.random(-50,50), skillLevel = 3, XPLevel = 0, equipLevel = 0, loginTime = 18, logoutTime = 15}		
